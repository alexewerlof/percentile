...
export function createIsGoodFnBody(sli, slo) {
    let fnBody = []
    switch (sli.upperBoundType) {
        case 'lte':
            fnBody.push(`if (dataPoint > ${slo.upperBoundThreshold}) return false`)
            break
        case 'lt':
            fnBody.push(`if (dataPoint >= ${slo.upperBoundThreshold}) return false`)
            break
    ...
    switch (sli.lowerBoundType) {
        case 'lte':
            fnBody.push(`if (dataPoint < ${slo.lowerBoundThreshold}) return false`)
            break
        case 'lt':
            fnBody.push(`if (dataPoint <= ${slo.lowerBoundThreshold}) return false`)
            break
