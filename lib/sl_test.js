...
import { calculateSlsMetric, createIsGood, createIsGoodFnBody } from './sl.js'
...
Deno.test('createIsGoodFnBody()', () => {
    ...
    const expected = `if (dataPoint > 10) return false; if (dataPoint <= 5) return false; return true`
    assertEquals(createIsGoodFnBody(sli, slo), expected)
})
